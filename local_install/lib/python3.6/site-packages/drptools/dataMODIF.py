...
class DRPLoader(object):
    ...
    def __init__(self, drp_path):
        ...
        # tract and patch
#        self.tracts = self.get_tract_list()
#        self.patches = self.get_patch_list()
    ...
    def _load_generic_dataset(self, datatype):
        ...
        for dataset in self.datasetTypes:
            ...
            for dataId in ([{}] + [self.dataIds[key][0] for key in self.dataIds]):
                try:
#                    print("_load_generic_dataset : ",dataset)
...
class DRPCatalogs(DRPLoader):
    ...
    def _get_catalog(self, dataset, **kwargs):
        ...
        # Define dataIds for which data are to be retrieved
        dataIds_local_set=self.dataIds[dataset][:]
        print("INFO: check for partial dataIds list")

        # Check if dataIds set is reduced in config file (ie. kwargs)
        dataIds_new=dataIds_local_set[:]
        keyList=['dataIds','filter','tract','patch']
        bReducedList = False
        for key in keyList:
            if kwargs and key in kwargs:
                if key=="dataIds":
                    ...
                    tmp=[x for x in dataIds_new if x[key] in kwargs[key]]
                print("NEW DATAIDS : ",tmp)
                dataIds_new=tmp[:]
                bReducedList = True                
            # if dataIds are defined in config file, do not check tract/path/filter sets
            if key=='dataIds' and bReducedList: break 

        if bReducedList:
            self.fullCatalog = False
            dataIds_local_set=dataIds_new[:]
            self.dataIds_local=dataIds_local_set
            print("INFO: check for reduced dataIds list - %d remaining dataIds "%len(dataIds_local_set))
            
        #Compute size of catalog
        filenames = (self.butler.get(dataset + "_filename",
                                     dataId, immediate=True)[0]
                     for dataId in dataIds_local_set)
...
#        cat = self.butler.get(dataset, self.dataIds[dataset][0],
        cat = self.butler.get(dataset, dataIds_local_set[0],
...
#        catadic = {k: [] for k in sorted(self.dataIds[dataset][0].keys())}
        catadic = {k: [] for k in sorted(dataIds_local_set[0].keys())}
...
#        for i, dataid in enumerate(self.dataIds[dataset]):
        # Loop over dataIds in order to retrieve data from the catalog
        for i, dataid in enumerate(dataIds_local_set):
    ...
    def _load_catalog(self, catalog, **kwargs):
        ...
        # return if full catalog is not loaded (
        # in case of MT loading process -> add_new_columns should be run once the
        #    complete catalog is loaded
        if not self.fullCatalog: return
...
class DRPCatalogsMT():
    ...
    def __init__(self, drpLoader):

        # Initialize data dictionnaries
        self.drpLoader=drpLoader
